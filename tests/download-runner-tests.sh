...
testRunnerDownloadWithNonExistingVersion() {
  ...
  # Change the default interval and max try values.
  tempScript=$(mktemp)
  newRetryInterval=1
  newRetryMaxLimit=5
  sed "s/retryInterval=30/retryInterval=${newRetryInterval}/; s/retryMaxLimit=10/retryMaxLimit=${newRetryMaxLimit}/" "$runnerDownloadScriptPath" >"$tempScript"
...
oneTimeSetUp() {
  runnerDownloadScriptPath="./download-runner.sh"
  runnerDownloadScript="bash $runnerDownloadScriptPath"
}
